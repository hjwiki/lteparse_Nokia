...
handler = logging.FileHandler("/home/nokia/nifi33/jyc/pmdianxin/log.txt")
...
###输出文件
output_path = '/data/output/pm/dianxin_nokia/4g'
output_name = os.path.join(output_path,'nokia_4g_dx_'+d0+'.csv')
...
def deal_with_file(fname):
    with gzip.open(fname, 'rt') as ff:
        ...
        for i in range(1,len(lines)):
            kpi.append(deal_with_kpi(lines[i]))
...
if __name__ == '__main__':
    ...
    pool = Pool(10)
    for i in [3,11]:
        pm_path = '/data/esbftp/dianxin/pm/4G/NOKIA/OMC'+str(i)+'/'+d0[0:10]+'/'
        print(pm_path)
        if os.path.isdir(pm_path):
            for root,dirs,files in os.walk(pm_path):
                for f in files:
                    if d0 in f:
                        fs.append(os.path.join(pm_path, f))

    pool.map(deal_with_file,fs)
