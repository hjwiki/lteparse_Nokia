...
def deal_with_file(fname,kpi):
    ...
    with gzip.open(fname, 'rt') as ff:
        ...
        PMSetuplist=list(root)
        p={}
        PMSetuplist_=list(PMSetuplist)
        PMSetuplist_.sort(key=lambda i:int(i.attrib['interval']))
        for PMSetup in PMSetuplist_: #个别xml包含一个以上的15分钟报告
            d={}
            ...
            if interval<=15:
                p[starttime]={'d':d,'interval':interval,'d':d}
            else:
                for starttime_ in p:
                    #找到这个PMSetup时间范围能包含的一个PMSetup
                    if starttime<=starttime_ and interval>p[starttime_]['interval']:
                        d=p[starttime_]['d'];break
